...
3. User Guide
-------------
The framework class loading, routing, http parameters, views, sessions and error handling.
...
A url ending with a file extension, e.g. '.php' or '.html', are not passed to the framework, they are managed by the server as if there was no framework. The only exception to this rule is a request for index.php in the webapp root, which is handled as specified above.
